...
def get_gradient_histogram_vec_from_patch(
    ...
    for i in range(4):
        for j in range(4):
            magnitude = window_magnitudes[i * 4: (i + 1) * 4, j * 4: (j + 1) * 4]
            orientation = window_orientations[i * 4: (i + 1) * 4, j * 4: (j + 1) * 4]
            histogram, _ = np.histogram(orientation, bins = 8, range = (-np.pi - (10 ** -5), np.pi), weights = magnitude)
            wgh[(i * 32) + (j * 8): (i * 32) + (j + 1) * 8] = histogram.reshape(8, 1)
...
def get_feat_vec(
    ...
    # TODO: YOUR CODE HERE                                                      #                                          #
    #############################################################################
    mag_p = np.pad(magnitudes, feature_width // 2)
    ori_p = np.pad(orientations, feature_width // 2)
    c = c + feature_width // 2
    r = r + feature_width // 2
    x_c = r - (feature_width // 2 - 1)
    y_c = c - (feature_width // 2 - 1)
    magnitudes_patch = mag_p[x_c: x_c + feature_width, y_c: y_c + feature_width]
    orientations_patch = ori_p[x_c: x_c + feature_width, y_c: y_c + feature_width]
