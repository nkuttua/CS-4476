...
def calculate_projection_matrix(
    ...
    for i in range(N):
        ...
        v = points_2d[i][1]
        j = 2 * i
        M[j] = [-X, -Y, -Z, -1, 0, 0, 0, 0, u * X, u * Y, u * Z, u]
        M[j + 1] = [0, 0, 0, 0, -X, -Y, -Z, -1, v * X, v * Y, v * Z, v]

    one, two, three = np.linalg.svd(M)
    M = three[-1, :]
    M = M.reshape((3, 4))
...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    homogeneous_3d = np.hstack((points_3d, np.ones((points_3d.shape[0], 1))))
    homogeneous_2d = np.dot(homogeneous_3d, P.T)

    top = homogeneous_2d[:, :2]
    bottom = homogeneous_2d[:, 2:]
    projected_points_2d = top / bottom
...
def calculate_camera_center(M: np.ndarray) -> np.ndarray:
    ...
    one, two, three = np.linalg.svd(M)
    homogeneous_cc = three[-1]
    homogeneous_cc = homogeneous_cc / homogeneous_cc[3]
    cc = homogeneous_cc[:3]
