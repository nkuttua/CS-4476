...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    x = np.arange(-1 * mean, ksize - mean)
    gv = np.exp(-x ** 2 / (2 * sigma ** 2)) / (np.sqrt(2 * np.pi) * sigma)
    nv = gv / np.sum(gv)
    kernel = nv.reshape(ksize, 1)
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    ksize = 4 * cutoff_frequency + 1
    sigma = cutoff_frequency
    gv = create_Gaussian_kernel_1D(ksize, sigma)
    kernel = np.outer(gv, gv)
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    h = k // 2
    w = j // 2
    pad = np.pad(image, ((h, h), (w, w), (0, 0)), mode='constant')
    filtered_image = np.zeros_like(image)
    for x in range(m):
        for y in range(n):
            for z in range(c):
                r = pad[x : x + k, y : y + j, z]
                filtered_image[x, y, z] = np.sum(r * filter)
